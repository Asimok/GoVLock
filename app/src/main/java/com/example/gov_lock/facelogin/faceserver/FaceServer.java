package com.example.gov_lock.facelogin.faceserver;
...
import com.example.gov_lock.facelogin.model.FaceRegisterInfo;
import com.example.gov_lock.facelogin.util.ImageUtil;
...
//    private static List<FaceRegisterInfo> faceRegisterInfoList;
    private static List<FaceMessage> faceRegisterInfoList;
    ...
    public static String faceFeatureHttp,sucessPerson="",sucessPersonName="";
    ...
    private void initFaceList(Context context) {
        synchronized (this) {
            ...
            faceRegisterInfoList=RecognizeActivityTest.faceRegisterInfoList;
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        Log.d("aa", imgDir + File.separator + userName + IMG_SUFFIX);
                        ...
                        Log.d("aa", featureDir + File.separator + userName);
//TODO 此处获取到特征值

                        //存储提取的特征值
                        faceFeatureHttp= Base64.getEncoder().encodeToString(faceFeature.getFeatureData());

                        Log.d("dd", "注册的特征值     "+faceFeatureHttp);
                        ...
                        faceRegisterInfoList.add(new FaceMessage(faceFeature.getFeatureData(), userName,employeeNumber));
                        return true;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        //TODO 在特征库中搜索
        Log.d("cc","在特征库中搜索" );
//        Log.d("cc","faceRegisterInfoList.size()  CompareResult    "+faceRegisterInfoList.size() );

        if (faceEngine == null || isProcessing || faceFeature == null || faceRegisterInfoList == null || faceRegisterInfoList.size() == 0) {
            Log.d("cc","在特征库中搜索 return null" );
            return null;
        }
        ...
        //TODO 从后端获取该会议室当前时间获取人脸库

        for (int i = 0; i < faceRegisterInfoList.size(); i++) {

            tempFaceFeature.setFeatureData(faceRegisterInfoList.get(i).getFaceFeature());
            faceEngine.compareFaceFeature(faceFeature, tempFaceFeature, faceSimilar);
          //  Log.d("dd","for里   getFaceFeature     "+ Base64.getEncoder().encodeToString(faceRegisterInfoList.get(i).getFaceFeature()));
            Log.d("dd","for里   getName     "+ faceRegisterInfoList.get(i).getName());
        ...
        if (maxSimilarIndex != -1) {
            Log.d("ff","maxSimilarIndex  识别成功    "+faceRegisterInfoList.get(maxSimilarIndex).getName());

            //TODO 这里可以获取到识别成功的员工  员工号
            sucessPerson=faceRegisterInfoList.get(maxSimilarIndex).getEmployeeNumber();
            sucessPersonName=faceRegisterInfoList.get(maxSimilarIndex).getName();
